...
| 21 | 오프닝 대화(스토리) UI 작업 | 미착수(신규), **다음 세션 우선 착수로 순서 변경**(2026-08-10, 20번보다 먼저). Figma 대신 사용자가 이미지 목업(대사 컷)을 직접 전달하는 방식으로 진행 예정. 재사용 후보: `Assets/Scripts/UI/EndingScene/StoryDialogueController.cs`+`TypewriterText.cs`(배경+대사 리스트를 순서대로 타이핑, 엑시트/영웅 엔딩에서 이미 검증된 패턴) — 새 대화 시스템 대신 이걸 그대로 쓸 수 있는지부터 확인. 오프닝을 어디서 트리거할지(`GameSceneManager`에 아직 Opening 씬 없음, CharacterSelect 이후 vs MainGame 진입 시 오버레이)는 목업 받고 나서 결정. **필수** — 플레이어들이 이 게임을 코인 사기 게임이 아니라 주식 게임으로 오해하고 있어서 앞부분 스토리로 정정해야 한다는 피드백 (15번 튜토리얼의 "엑시트 목표 이유 설명"과도 연결됨) |
