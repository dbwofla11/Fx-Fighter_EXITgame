...
| 22 | 2차 피드백(빨간 글씨 항목) 처리 | **① 완료**(2026-08-12, `Issue_PriceShockSkills.md` 참고) — 신규 스킬을 만들지 않고 **기존 시장조작 9종을 전부 개편**하는 방향으로 다시 작업함(1차 시도였던 신규 스킬 2종은 되돌림). `baseCost` 전체 인상(구 22,500~90,000 → 신규 50,000~200,000, 여론조작 카테고리보다 비싼 축으로), Doubt 대체로 2배 이상, Support/Growth/Volume도 1.3~1.6배로 올려 하이리스크·하이리턴화. 그중 `펌핑`(+20%)·`허수매도벽`(-20%)에 신규 `EffectType.PriceShockPercent`를 얹어 "가격 즉시 변동"을 기존 스킬 확장으로 구현(펌핑=즉시 급등, 허수매도벽=즉시 폭락 후 저가 매수 컨셉). **스킬 패널 UI 쪽은 손대지 않음** — 기존 9개 아이콘 버튼을 그대로 재사용하는 구조라 이번 변경은 수치만 바뀌고 화면엔 자동 반영됨. ②③ 미착수, 둘 다 착수 전 확인 필요해서 보류: ② 엑시트 당위성/빚 스토리(엄마 5억 빚·비행기 도피·매년 이자 오버로드 — 20번 목표금액 오버레이·21번 오프닝 스토리와 내용 겹침, 21번은 사용자가 이미지 목업 줄 때까지 대기 중이라 같이 대기) → ③ 스탯 슬라이드(의심도/지지도/상승률) 호버 툴팁(Notion 원문에 "Figma 정리 필요, 준원이형 파트"로 명시돼 있어 내 작업 범위 아닐 수 있음 — 착수 전 확인 필요). Notion ["2차 피드백 정리"](https://app.notion.com/p/3b694f5130278048a560d0b4232e7bfe) 참고. |
