...
// 엔딩 결과 디스패처(비주얼 없음). EventHub.OnGameEnded(EndingType)를 구독해 3종 엔딩(체포/엑시트/거지)을
// 각자 전용 씬으로 넘긴다 — 체포/거지는 EndingScene(EndingSceneUI), 엑시트는 ExitEndingScene
...
public class EndingResultUI : MonoBehaviour
{
    ...
    private void HandleGameEnded(EndingType ending)
    {
        ...
        // 보이도록 기다린 뒤 넘어간다. 엑시트는 그런 붕괴 연출이 없어 바로 넘어간다.
