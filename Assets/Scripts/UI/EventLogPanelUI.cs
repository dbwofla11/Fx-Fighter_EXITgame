...
// node 1253:2 "뉴스,이벤트 페이지 - 스탯개요"(개요 탭 콘텐츠, 아직 미구현)와
// node 1261:195 "뉴스,이벤트 페이지 - 이벤트 패널"(커뮤니티 탭 콘텐츠, 아래 eventPanelBox).
// 탭/닫기버튼/ContentArea/eventPanelBox/카드 템플릿은 전부 씬에 직접 배치된 오브젝트를 참조한다
...
// "+/-" 버튼을 만든 것과 동일한 방식).
// "개요" 탭(스탯개요 + 엑시트 버튼)은 아직 설계/구현 전이라 자리만 잡아두고 내용은 비워둔다
// (Next_Tesk.md "엑시트 버튼" 항목과 통합될 예정).
public class EventLogPanelUI : MonoBehaviour
{
    ...
    private void ShowOverview() // 스탯개요/엑시트 버튼 미구현이라 콘텐츠 없음(빈 화면)
    {
